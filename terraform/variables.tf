...
// Read from "TF_VAR_WEBAPP_RESOURCE_GROUP_NAME" environment variable, which should be set in the .env file
variable "WEBAPP_RESOURCE_GROUP_NAME" {
  description = "The name of the Azure Resource Group where the web application resources will be created."
  type        = string

  validation {
    condition     = var.WEBAPP_RESOURCE_GROUP_NAME != null || var.WEBAPP_RESOURCE_GROUP_NAME != ""
    error_message = "WEBAPP_RESOURCE_GROUP_NAME cannot be null or empty. Please provide a valid resource group name for the web application."
  }
}
